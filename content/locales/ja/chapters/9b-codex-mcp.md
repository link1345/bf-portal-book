...
第9章では、完成したエクスペリエンスを公開し、説明し、壊さず更新するための運営手順を整理しました。けれど、公開後に不具合を直したり、新しい要素を足したりするとき、毎回すべてのAPIや型を人間だけで追うのは重くなります。
本章では、Portalコードを書く作業を **AIに任せるための渡し方** を整理します。やりたいこと、対象マップ、対象オブジェクト、発火タイミング、実機で起きた現象、`PortalLog.txt` の内容を、AIが使える材料として渡す流れを作ります。

ここでは、AIに丸投げして終わりにするのではなく、後の修正作業にそのまま使える形で、役割分担、質問の書き方、ログの渡し方、MCPの使いどころ、実機確認までの関係を決めていきます。
...
MCPは、AIに渡す辞書のようなものです。人間がMCPを直接操作してAPIを暗記するというより、AIに「分からないところはMCPで調べて」と頼むための道具だと考えると分かりやすいです。

ただし、MCPだけで実機の挙動までは分かりません。ゲーム内でどうなったか、ログに何が出たかは、人間がAIに渡します。

MCPの設定や使い方で詰まった場合も、まずCodexのAIにそのまま聞いて構いません。

```text
bf6_portal_typescript_mcpを設定したいです。
今の設定画面とエラーを見て、何が足りないか確認してください。
```

```text
Portal APIで分からない点があれば、MCPを使って調べてから回答してください。
```

AIに聞くときは、エラー文、設定画面の入力内容、SDKフォルダの場所を一緒に渡すと話が早くなります。
...
設定しても動かない場合は、次の順番で確認します。

* `npx` が使える状態になっているか。
* `--sdk_path` に指定したフォルダの中に `code/types/mod/index.d.ts` があるか。
* 設定後にCodex Appを再起動したか。
* Codexのツール一覧に `bf6_portal_typescript_mcp` が出ているか。
* サーバー名が短く分かりやすい名前になっているか。

サーバー名が分かりにくいと、AIがどのMCPを使えばよいか迷う場合があります。
OpenAIのDocs MCPの説明でも、必要なMCPを使わせたい場合は、AIへの明示指示や `AGENTS.md` への記載が案内されています。

https://developers.openai.com/learn/docs-mcp

## Codex Appの設定も少し見ておく

Codex AppのSettingsでは、MCPだけでなく、外観、パーソナリティ、カスタム指示なども調整できます。
日本語で使いたい場合は、カスタム指示に次のように書いておくと、説明は日本語に寄せつつ、コードやログは壊しにくくなります。

```text
日本語で説明してください。
コード、関数名、ログは原文の英語を保ってください。
```

コード、関数名、ログまで日本語に翻訳されると、Portal側でそのまま使えない内容になることがあります。
説明文は日本語、コードとAPI名は英語のまま、という分け方にしておくのが無難です。
...
これでは、読み込み失敗なのか、イベント未発火なのか、座標ミスなのか、オブジェクト違いなのかが分かりません。
...
## MCPを設定したのにAIが使ってくれない

AIは、使える道具があっても、毎回自動で最適に選んでくれるとは限りません。
Portal APIで不明点があるときは、依頼文に「MCPを使って調べてください」と明示します。

```text
bf6_portal_typescript_mcpが使えます。
Portal APIやenumで分からない点は、MCPで確認してからコードを書いてください。
```

毎回書くのが面倒なら、プロジェクトの `AGENTS.md` に同じ方針を書いておくのも有効です。

## エラー文が英語で読めない

エラー文は、無理に自分で翻訳しなくても構いません。
そのままAIに貼って、日本語で原因と次の確認手順を聞きます。

```text
次のエラーが出ました。
英語が苦手なので、日本語で原因と確認手順を説明してください。
ただし、コードや関数名は英語のまま残してください。

...
```

エラー文は省略しすぎると原因が消えます。長すぎる場合でも、最初のエラー、最後のエラー、自分が実行した操作は残した方がよいです。

## Codexの返答が英語になる

Codex AppのSettingsやカスタム指示で、日本語回答を指定します。
ただし、Portalのコード、API名、ログ、ファイル名は英語のまま残すように書いておきます。

日本語にしてよいのは説明です。コードの中身まで日本語にすると、コピーしたときに壊れることがあります。

## レート制限が怖い

OpenAIのレート制限は、リクエスト数やトークン数など複数の軸で測られ、モデルによっても変わります。

https://developers.openai.com/api/docs/guides/rate-limits

`fast` は「雑に大量消費してよいモード」ではありません。
返答が速いぶん、短時間に何度も依頼しやすくなります。さらに、長いログや長いコードを毎回まるごと貼ると、トークン消費が大きくなり、制限に近づきやすく感じます。

怖い場合は、次のように分けます。

* ログは関係ありそうな部分から貼る。
* 大きな修正を一度に頼まず、「まず原因だけ見て」と頼む。
* コード全文を毎回貼らず、「前回から変えた部分」と「出たエラー」を中心に渡す。
* 何度も同じ説明を貼らず、目的、現象、ログ抜粋、お願いの形に整理する。

速いモードは便利ですが、節約モードだと思い込まない方が安全です。
