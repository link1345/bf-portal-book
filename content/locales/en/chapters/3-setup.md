...
In the local SDK's `sdk.version.json`, the reference SDK version is `1.3.3.0`. The SDK changes through updates, so if this book's steps differ from your screen, first check `sdk.version.json` and `docs/pages`.
