...
**地圖上可以放置的物件，僅限於 Godot 檔案系統 `res://objects` 內的物件。** 此外，**根據以哪張地圖為基礎進行編輯，可放置物件的範圍也會受到限制。** **截至 2026 年 6 月 12 日，手頭的 Portal SDK（版本：1.3.2.0）結構如下。**
...
SDK 的 `docs/pages/spatial_editor.html` 和 `FbExportData/level_info.json` 中，可用 Level 整理如下（截至 2026 年 6 月 12 日，SDK 1.3.2.0）。
...
`RuntimeSpawn_Common` 是較容易在多個 Map 中使用的通用類，`RuntimeSpawn_Abbasid` 等帶有 Map 名稱的內容，則理解為來自該 Map 的候選。
