...
:::message alert
ここからTypeScriptを使ったプログラミングが入りますが、プログラム中に **絶対に日本語などのマルチバイト文字を入れない** でください。
2025年11月1日時点では、PortalのScript機能は、日本語などのマルチバイト文字に対応していません。アルファベット・数字・一部の記号のみを使用してください。
ChatGPTなどのAIにコードを書かせると、ハイフンに見えるダッシュ（`—`、`–`）や、普通の引用符に見えるスマートクォート（`“`、`”`、`‘`、`’`）などの特殊なUnicode文字が混ざることがあります。Portal側ではこれらも問題になるため、貼り付ける前にASCIIの `-`、`"`、`'` に直すか、使わない形に書き換えてください。
