...
:::message
When looking for placeable objects, use [BF6 Object Guide](https://bf6-book.orizika.com/) as a companion reference.
...
**Objects you can place on a map are limited to files inside Godot's `res://objects` file system.** In addition, **the range of placeable objects depends on which map you use as the base for editing.** **As of July 1, 2026, the Portal SDK available here (version 1.3.3.0) is structured as follows.**
...
In the SDK's `docs/pages/spatial_editor.html` and `FbExportData/level_info.json`, the available Levels are organized as follows (as of July 1, 2026, SDK 1.3.3.0).
...
Note: SDK 1.3.3.0 adds the community map `MP_Aftermath_Portal` and the Spatial Editor map `MP_Plaza`. Runtime spawn candidates from `MP_Plaza` are also added to the type definitions as `RuntimeSpawn_Plaza`.
...
`RuntimeSpawn_Common` is a common set that is easier to use across multiple maps. Names such as `RuntimeSpawn_Abbasid` and `RuntimeSpawn_Plaza` should be read as candidates derived from that map.
...
In SDK 1.3.3.0, `EnableSpatialObject` has been removed. If you want to add or remove placed objects during play, design around `SpawnObject` and `UnspawnObject`.
...
## Bomb / MCOM (Obliteration-style bomb objectives)

SDK 1.3.3.0 adds the `Bomb` type and bomb-linked M-COM settings for Obliteration.
You can place a Bomb in the Spatial Editor, or spawn one with `mod.SpawnObject(mod.RuntimeSpawn_Common.Bomb, ...)`.

On the TypeScript side, get it with `mod.GetBomb(id)` and control it with `mod.GiveBombToPlayer(...)`, `mod.ForceBombDrop(...)`, `mod.ForceBombReset(...)`, `mod.SetBombTeam(...)`, `mod.SetBombDropFuseTime(...)`, and related functions.
For M-COM, `mod.SetMCOMArmType(mod.GetMCOM(id), mod.MCOMArmType.Bomb)` makes the objective armable only by the bomb carrier.

## MovingPlatform

SDK 1.3.3.0 adds `MovingPlatform` assets for moving platforms.
The sample either spawns `BarrierStoneBlock_01_H_PortalPlatform` with `SpawnObject`, or places it in Godot with an ObjId, then moves it with `MoveObjectOverTime` or `OrbitObjectOverTime`.

For a platform players stand on, collision and replication matter as much as visual movement.
Instead of forcing ordinary decorative objects to move, choose a MovingPlatform-compatible asset and record its travel distance, period, reverse behavior, and stop timing in your ledger.
...
SDK 1.3.3.0 also adds the `VehicleResupplyStation` asset for vehicles.
When treating it as a placement candidate, check vehicle approach paths, enough room to avoid blocking during resupply, and the surrounding combat pressure.
