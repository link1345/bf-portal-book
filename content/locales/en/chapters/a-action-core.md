---
title: "Appendix A-2: Action/value acquisition list (basic operations)"
free: true
---
...
| Function | Function/Purpose | Main argument | Return value | Usage example |
| --- | --- | --- | --- | --- |
...
| `SpawnAIFromAISpawner` | Spawns an AI soldier from the specified AI Spawner. 8 types of overlords. | spawner: Spawner | None | `mod.SpawnAIFromAISpawner(mod.GetSpawner(0));` |
...
| `AIEnableShooting` | Switch whether AI soldiers can shoot or not. For AI players only. 2 types of overlord. | player: Player | None | `mod.AIEnableShooting(...);` |
| `AIEnableTargeting` | Switch AI soldiers' enemy search/target recognition. If you disable it, it will no longer fire. 2 types of overlord. | player: Player | None | `mod.AIEnableTargeting(...);` |
...
| `AISetTarget` | Set or clear the current target for AI soldiers. 2 types of overlord. | aiPlayer: Player, targetPlayer: Player | None | `mod.AISetTarget(...);` |
| `AIStartUsingGadget` | Have an AI soldier use a specific gadget at a specified location or player. 2 types of overlord. | player: Player, gadget: Gadgets, targetPos: Vector | None | `mod.AIStartUsingGadget(...);` |
...
| `PlayMusic` | Play music events. You can target the entire team, squad, or players. 4 types of overlords. | musicEvent: MusicEvents | None | `mod.PlayMusic(...);` |
| `PlaySound` | Play SFX. You can specify the target, location, and attenuation range. 8 types of overlords. | sound: SFX, amplitude: number, team: Team | None | `mod.PlaySound(mod.GetSFX(0), 1);` |
| `PlayVO` | Play VO audio events. You can target the entire team, squad, or players. 4 types of overlords. | voiceOver: VO, event: VoiceOverEvents2D, flag: VoiceOverFlags | None | `mod.PlayVO(...);` |
| `SetMusicParam` | Update the parameter values of the loaded music package. 4 types of overlords. | musicParam: MusicParams, paramValue: number | None | `mod.SetMusicParam(...);` |
| `SetSoundAmplitude` | Change the volume of the specified SFX. You can target the entire team, squad, or players. 4 types of overlords. | sound: SFX, amplitude: number, team: Team | None | `mod.SetSoundAmplitude(...);` |
| `StopSound` | Stop the specified SFX. You can target the entire team, squad, or players. 4 types of overlords. | sound: SFX, team: Team | None | `mod.StopSound(...);` |
...
| Function | Function/Purpose | Main argument | Return value | Usage example |
| --- | --- | --- | --- | --- |
| `SetCameraTypeForAll` | Set the camera type for all players. If necessary, also specify the camera index. 2 types of overlord. | cameraType: Cameras | None | `mod.SetCameraTypeForAll(...);` |
| `SetCameraTypeForPlayer` | Set the camera type of the specified player. If necessary, also specify the camera index. 2 types of overlord. | player: Player, cameraType: Cameras | None | `mod.SetCameraTypeForPlayer(...);` |
...
| `SetEmplacementSpawnerTimeUntilAbandon` | Sets the amount of time a stationary weapon remains unused until it becomes abandoned. | `SetEmplacementSpawnerTimeUntilAbandon` | | emplacementSpawner: EmplacementSpawner, timeUntilAbandon: number | None | `mod.SetEmplacementSpawnerTimeUntilAbandon(...);` |
...
| `SpawnPlayerFromSpawnPoint` | Sortie the specified player from the specified SpawnPoint. 2 types of overlord. | player: Player, spawnPointId: number | None | `mod.SpawnPlayerFromSpawnPoint(eventPlayer, mod.GetSpawnPoint(0));` |

## Game Progress/Team

| Function | Function/Purpose | Main argument | Return value | Usage example |
| --- | --- | --- | --- | --- |
| `EndGameMode` | Ends the game mode and makes the specified player or team the winner. Team 0 is a draw. 2 types of overlord. | player: Player | None | `mod.EndGameMode(mod.GetTeam(1));` |
...
| `SetGameModeScore` | Set the game mode score for a specified player or team. 2 types of overlord. | team: Team, newScore: number | None | `mod.SetGameModeScore(mod.GetTeam(1), 10);` |
...
| `GetGameModeScore` | Get the game mode score for a specified player or team. 2 types of overlord. | player: Player | `number` | `const value = mod.GetGameModeScore(...);` |
...
| `RayCast` | Request straight line judgment between two points. Receive the results at `OnRayCastHit` / `OnRayCastMissed`. 2 types of overlord. | player: Player, start: Vector, stop: Vector | None | `mod.RayCast(eventPlayer, start, stop);` |
...
| `MoveObject` | Moves the specified object by position difference, and rotation difference if necessary. 2 types of overlord. | object: mod.Object, positionDelta: Vector | None | `mod.MoveObject(...);` |
...
| `OrbitObjectOverTime` | Rotates the specified object around the Transform over time. 2 types of overlord. | object: mod.Object, orbitTransform: Transform, timeInSeconds: number, radius: number, shouldLoop: boolean, shouldReverse: boolean, clockwise: boolean | None | `mod.OrbitObjectOverTime(...);` |
...
| Function | Function/Purpose | Main argument | Return value | Usage example |
| --- | --- | --- | --- | --- |
| `SpawnLoot` | Spawn ammo, weapons, gadgets, and armor in LootSpawner. 4 types of overlords. | lootSpawner: LootSpawner, ammo: AmmoTypes | None | `mod.SpawnLoot(...);` |
...
| Function | Function/Purpose | Main argument | Return value | Usage example |
| --- | --- | --- | --- | --- |
| `DealDamage` | Cause damage to the designated player or vehicle. If necessary, you can also specify the player who caused the damage. 3 types of overlords. | player: Player, damageAmount: number | None | `mod.DealDamage(...);` |
| `ForceRevive` | Forcibly revive a downed player. | player: Player | None | `mod.ForceRevive(...);` |
| `Heal` | Heal or repair the designated player or vehicle. If necessary, you can also specify a healer. 3 types of overlords. | player: Player, healAmount: number | None | `mod.Heal(...);` |
| `Kill` | Instantly kill the designated player or vehicle. Player skips down state. 2 types of overlord. | player: Player | None | `mod.Kill(...);` |
...
| `SpotTarget` | Spot the specified player. You can specify the display time, spot status, and spotter. 5 types of overlords. | targetplayer: Player, duration: number, spotStatus: SpotStatus | None | `mod.SpotTarget(...);` |
...
## Equipment/input/player operation

| Function | Function/Purpose | Main argument | Return value | Usage example |
| --- | --- | --- | --- | --- |
| `Teleport` | Move the specified player or vehicle to valid coordinates with orientation. Specify the direction in radians. 2 types of overlord. | player: Player, destination: Vector, orientation: number | None | `mod.Teleport(eventPlayer, destination, 0);` |
...
| `AddEquipment` | Add weapons, gadgets, and armor to the player's loadout. 7 types of overlords. | player: Player, weapon: Weapons | None | `mod.AddEquipment(...);` |
...
| `RemoveEquipment` | Removes the specified slot, weapon, or gadget from the player's loadout. 3 types of overlords. | player: Player, inventorySlot: InventorySlots | None | `mod.RemoveEquipment(...);` |
...
| `Resupply` | Supply the player with the specified supply type. | player: Player, resupplyType: ResupplyTypes | None | `mod.Resupply(...);` |
...
| `EventWeaponCompare` | Determine whether the weapons and gadgets given in the event match the specified equipment. 2 types of overlord. | eventWeapon: WeaponUnlock, weapon: Weapons | `boolean` | `if (mod.EventWeaponCompare(eventWeaponUnlock, mod.Weapons.AK24)) {}` |
...
| `GetSoldierState` | Obtain the specified player's soldier status as a numerical value, boolean value, or Vector. 3 types of overlords. | player: Player, soldierStateNumber: SoldierStateNumber | `number` | `const value = mod.GetSoldierState(...);` |
| `HasEquipment` | Determine whether the specified player has the specified weapon or gadget. 2 types of overlord. | player: Player, weapon: Weapons | `boolean` | `if (mod.HasEquipment(eventPlayer, mod.Weapons.AK24)) {}` |
...
| `ForcePlayerExitVehicle` | Forcibly remove the designated player from the vehicle. | `ForcePlayerExitVehicle` | 3 types of overlords. | player: Player, vehicle: Vehicle | None | `mod.ForcePlayerExitVehicle(...);` |
| `ForcePlayerToSeat` | Force the designated player to sit in the designated vehicle seat. | `ForcePlayerToSeat` | If seat number -1, use the vacant seat. | player: Player, vehicle: Vehicle, seatNumber: number | None | `mod.ForcePlayerToSeat(...);` |
...
| `RemoveUIIcon` | Delete the UI Icon Widget attached to the object. You can also specify the target player or team. 2 types of overlord. | objectWithIcon: mod.Object | None | `mod.RemoveUIIcon(...);` |
...
| `SetWorldIconOwner` | Restrict WorldIcon to be visible only to specific teams or players. 2 types of overlord. | worldIcon: WorldIcon, newTeamOwner: Team | None | `mod.SetWorldIconOwner(...);` |
...
UI, notifications, player references, arrays, ObjId, numbers, types, and minimum templates are divided into the following "Appendix A-3: List of actions/value acquisition (UI/references/values)".
