...
:::message alert
从这里开始会进入使用 TypeScript 的编程，但请**绝对不要在程序中写入日语等多字节字符**。
截至 2025 年 11 月 1 日，Portal 的 Script 功能不支持日语等多字节字符。请只使用字母、数字和一部分符号。
让 ChatGPT 等 AI 编写代码时，可能会混入一些特殊 Unicode 字符，例如看起来像连字符的破折号（`—`、`–`），或看起来像普通引号的智能引号（`“`、`”`、`‘`、`’`）。这些字符在 Portal 侧也会造成问题，所以粘贴之前请替换为 ASCII 的 `-`、`"`、`'`，或者改写成不使用这些字符的形式。
